...
class EvaluationUtils:
    ...
    def detection_performance_2(self):
        ...
        tn_reqs = sum([sum([hosts[1] for hosts in fingerprint.hosts])
                       for fingerprint in tn])
        fp_reqs = sum([sum([hosts[1] for hosts in fingerprint.hosts])
        ...
        retrained_fp_reqs = sum(
            [sum([hosts[1] for hosts in fingerprint.hosts]) for fingerprint in retrained_fp])
