...
class DetectionModule():
    ...
    def __init__(self):
        ...
        self.known_browsers_str = ['Firefox', 'Chrome',
                                   'MSIE', 'Edge', 'Opera', 'Safari']  # TODO
    ...
    def _is_update(self, alert_fingerprint, trained_fingerprints):  # TODO
        ...
        for fingerprint in trained_fingerprints:
            ...
            if alert_fingerprint.method == fingerprint.method:
                ...
                ua_similarity_distance = self._ua_distance(
                    alert_fingerprint.user_agent[0], fingerprint.user_agent[0])
    ...
    def _similar_alert(self, alert, fingerprint):
        ...
        if alert.label == fingerprint.label:
            ...
            if alert.label == "Background":
                score += self._header_check(alert.constant_header_fields,
                                            fingerprint.constant_header_fields)
                score += self._avg_size_check(alert.avg_size,
                                              fingerprint.avg_size)
                score += self._host_check(alert.hosts, fingerprint.hosts)
                if score >= self.background_threshold:
                    similar = True
                    return similar
            elif alert.label == "Browser":
                score += self._language_check(alert.language,
                ...
                raise AttributeError(
                    "The label of the alert is neither Browser nor Background.")
        return similar
    ...
    def _background_similarity(self, new_f1, old_f2):
        ...
        score += self._header_check(new_f1.constant_header_fields,
                                    old_f2.constant_header_fields)
