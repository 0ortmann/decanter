...
class Fingerprint():
    ...
    def __init__(self, label, ua, hosts, ip_dsts, const_head, lang,
        ...
        if label == "Background":
            ...
            raise ValueError(
                'The label passed %s is not a "Browser" or "Background".' % (label))
...
class FingerprintGenerator():
        ...
        for http_request in method_cluster:
            ...
            if not cache:
                ...
                for header_name in http_request.header_values.keys():
                    ...
                    total_size_headers += len(
                        http_request.header_values[header_name])
    ...
    def _compute_outgoing_info(
        ...
        outgoing_info += self._levenshtein_distance(
            current_req.uri, old_req.uri)
        outgoing_info += current_req.req_body_len
