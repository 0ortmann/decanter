...
class ReferrerGraph:
    ...
    def appendable(self, cluster):
        ...
        appended_graph = ReferrerGraph(
            full_cluster, self.subdomains, self.time_threshold)
    ...
    def _isLinked_(self, request, headNode):
        ...
        referrer = request.header_values.get(
            'referer', request.header_values.get('origin', ''))
...
class LabelGenerator():
    ...
    def generate_label(self, cluster, mode=1, browser_user_agents=set(), referrerGraphs=dict()):
        ...
        # Suspecter Browser are clusters labelled as Background applications, and have a known "browser" user-agent
            ...
            g = referrerGraphs.get(
                cluster[0].header_values.get('user-agent', None), None)
    ...
    def _check_browser_malware(self, referrerGraph):
        ...
        malicious_attempts = self._exfiltration_similarity_threshold(
            exfiltration_attempts, similarity_attempts)
    ...
    def _header_filter(self, referrerGraph, connections, threshold=500):
        ...
        for key, value in connections.items():
            ...
            s_headers = map(lambda req: sorted(
                req, key=lambda tup: tup[0]), headers)
            val = map(lambda req: [list(elem)
                                   for elem in zip(*req)], s_headers)
