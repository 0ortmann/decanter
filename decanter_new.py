...
class Aggregator:
    ...
    def _create_fingerprints(self, host, http_cluster):
        ...
        labels, referrerGraph = self.label_generator.generate_label(
            http_cluster, self.mode, self.browser_user_agents, self.referrerGraphs)
        ...
        if self.mode == 0:
            ...
            for key, value in labels.items():
                ...
                self.fin_manager.store(
                    host, self.fin_generator.generate_fingerprint(cluster, method, label))
        ...
        if req.orig_ip in self.hosts_clusters:
            ...
            # Create and/or Update a cluster for those requests that DO HAVE a User-Agent
                ...
                if req.header_values['user-agent'] not in self.hosts_clusters[req.orig_ip]:
                    ...
                    self.hosts_clusters[req.orig_ip][req.header_values['user-agent']].append(
                        req)
